...
@app.route('/admin/book/<id>', methods=['GET', 'POST'])
def manager_book(id):
    manager_judge()
...
def manager_books_add():
    ...
    error = None
    if request.method == 'POST':
        if not request.form['BookNum']:
            error = 'You have to input the book id'
        elif not request.form['BookName']:
            error = 'You have to input the book name'
        elif not request.form['Writer']:
            error = 'You have to input the book writer'
